...
| TR-007 | CAN | Var | Var | KEEP functionally; exact Classic CAN/CAN-FD contract OPEN | FC protocol/failsafe + `CONTROL_PLATFORM_BENCH_CONTRACT.md` |
...
| TR-020 | Hardware trip | CMPSS/Trip Zone + driver | dedicated trip logic | KEEP principle + REVALIDATE; common candidate bench contract requires asynchronous fault-to-PWM-inactive measurement and safe reset behavior. | `CONTROL_PLATFORM_BENCH_CONTRACT.md`, final latency/threshold/reset budget |
...
- Gate-driver voltage-domain trade gerçek alternatifler içeriyor: UCC27282 120 V bootstrap sınıfında olsa da TI HS normal çalışma tavanı 100 V olduğu için >100 V switch-node çözümü sayılmıyor; UCC27712 620 V non-isolated half-bridge ve UCC21540-Q1 reinforced-isolated driver daha yüksek domain alternatifleri olarak kaydedildi. Bu parçalar adaydır, seçim değildir.
- Auxiliary-power trade LM5164 dışında iki yüksek-domain anchor içeriyor: LTC3639 4–150 V / 100 mA housekeeping sınıfı ve LTC7801 4–140 V operating / 150 V abs-max controller. Final seçim için toplam gate-drive/control/fan load budget gerekir.
- `B1_AUXILIARY_LOAD_DOMAIN_INVENTORY.md` legacy B1 güç ağacını `VBUS -> 12V -> 5V -> 3V3 -> 3V3A` olarak izledi ve görünür tüketicileri ayırdı. LM5164/TPS62160/TLV755 headline akım değerleri kaynak kapasitesi olarak tutuldu; U1 yük talebi olarak kullanılmıyor. B1 fan <=0.2 A değeri yalnız legacy başlangıç bütçesi, fan inrush ise ölçüm gerektiriyor. Üç pasif trip-reference bölücüsünün toplamı yaklaşık 0.371 mA; bu yalnız pasif alt toplamdır, analog rail bütçesi değildir.
- `CONTROL_PLATFORM_PRETRADE.md` exact STM32G474RE/RET3 ve TMS320F280041C/F280041CPMS kaynaklarını karşılaştırdı; seçim yapmadı. `CONTROL_PLATFORM_BENCH_CONTRACT.md` iki aday için aynı PWM/ADC/fault/CAN/FOC workload'unu ve CPB-01..12 ölçüm sözleşmesini tanımladı. FOC runtime ve fault latency fiziksel/çalıştırılabilir kanıt olmadan iddia edilmiyor.
- `U1_BOM_CANDIDATES.json` driver/aux alternatiflerini `CANDIDATE` olarak izliyor; null/Open güç konnektörü, DC-link, precharge, fuse ve clamp satırları açık kalıyor.
