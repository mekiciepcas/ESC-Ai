...
- U8 Lite KV85 controller-timing trade evidence now includes a **67.2 keRPM / 1.12 kHz electrical** published-curve-derived anchor using the separately identified G28x9.2 CF curve; it is not an exact selected-prop operating point.
...
- The 3 kW/36 V frozen parents impose an **83.33 A propulsion-only continuous pack-current lower bound**; final continuous current follows the TR-067 closure contract and remains OPEN until auxiliary load and margin are controlled.
