...
| TR-007 | CAN | Var | SN65HVD230DR / optional 120R / TVS placeholder | KEEP functionally; exact Classic CAN/CAN-FD, temperature, termination, TVS/EMC and harness contract OPEN. B1 exact transceiver remains REVALIDATE. | `B1_LEGACY_TRACEABILITY_GAP_AUDIT.md`, `EXTERNAL_AUX_INTERFACE_CONTRACT.template.json`, FC protocol/failsafe + bus test |
...
- `B1_LEGACY_TRACEABILITY_GAP_AUDIT.md` now closes the evidence classification for fault latch, PWM inhibit, hardware comparators, CAN, temperature, Hall and board interfaces. The B1 external E-stop contact cannot be credited with cable-break detection, and the legacy Hall/motor-temperature WR-PHD headers are PoC references rather than production UAV connectors.
- `EXTERNAL_AUX_INTERFACE_CONTRACT.template.json` now holds fan, Hall, exported +5V, motor-temperature, E-stop/inhibit, CAN and control/power board-interconnect fields with all unknown product values kept null.
...
- `U1_BOM_CANDIDATES.json` now tracks exact legacy support parts `SN74LVC1G74DCUR`, `SN74LVC1G08DBVR`, `TLV1704PWR`, `NTCG203NH103JT1`, `SN65HVD230DR`, `61300511121` and `61300211121` as REVALIDATE or LEGACY_REFERENCE only; none is promoted to production selection.
