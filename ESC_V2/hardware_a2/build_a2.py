"""A2 engineering review schematics. Builds from A1 circuit source without running its exporter.
...
for p in parts:
    ...
    if p['kind']=='CONN40':p['nets']['39']='+5V';p['note']+=' A2 pin 39 now carries +5V for control-board Hall supply.'
    if p['kind']=='DRV':p.update(footprint='ESC_A2:TI_RTA0040B_6x6_P0.5_EP4.15',note='TI DRV8353F Rev B. VM=12V, VDRAIN=DC link. SPI variant; configure 6-PWM and CSA gain10. Local RTA0040B land pattern from TI drawing; assembler review pending.')
...
page('09_AUX_LOGIC','12 V → 5 V → 3.3 V / analog besleme',aux,[
 ...
 '5 V hattı A2 kart arayüzünde pin39 üzerinden kontrol kartına taşınır. A1 arayüz eksiği kapatıldı.'
])
mc=original['08_MCU']
...
page('18_BOARD_INTERFACE','Güç / kontrol kartı arayüzü',original['12_board_interface'],[
 ...
 'A2 düzeltme: pin39 +5V; pin40 GND. Pin1 ve bakış yönü fiziksel montaj çiziminde sabitlenecek.',
...
def base(name,title):return f'(kicad_sch (version 20250114) (generator "eeschema") (uuid {q(uid(name))}) (paper "A3") (title_block (title {q(title)}) (date "2026-09-13") (rev "A2 REVIEW") (company "ESC V2 | AR-GE") (comment 1 "TASARIM INCELEMESI / URETIM SERBEST BIRAKMA DEGIL") (comment 2 "39-54.6V | 3kW mil hedefi | 20kHz | 13S Li-ion"))'
...
for key,pageinfo in pages.items():
    ...
    s+=text(title,15,14,2.3)+text('ESC-3K | '+key+' | A2 | TASARIM INCELEMESI',15,23,1.2)
...
s+=text('ESC 3 kW | A2 MÜHENDİSLİK İNCELEMESİ',15,15,2.8)
...
(HERE/'sym-lib-table').write_text('(sym_lib_table (lib (name "ESC_A2") (type "KiCad") (uri "${KIPRJMOD}/ESC_A2.kicad_sym") (options "") (descr "A2 pin-numbered engineering symbols")))',encoding='utf-8')
(HERE/'ESC_A2.kicad_sym').write_text('(kicad_symbol_lib (version 20241209) (generator "kicad_symbol_editor")'+''.join(sym(k)[0].replace(q(LIB+':'+k),q(k),1) for k in sorted({p['kind'] for p in parts}))+')',encoding='utf-8')
...
print(f'A2: {len(pages)+1} pages; {len(parts)} symbols; {sum(len(p["parts"]) for p in pages.values())} placed.')
