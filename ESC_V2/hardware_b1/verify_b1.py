...
netlist=parse((root.parent/'verification/ESC_3kW_B1.net').read_text(encoding='utf-8'))
actual={};nets={}
...
check('LM5164 RON resistor returns to GND, not VBUS',actual.get(('R1301','1'))=='GND' and actual.get(('R1301','2'))=='BUCK_RON')
for c in kinds['CONN40']:check(c['ref']+' Hall supply transferred on pin39',actual[c['ref'],'39']=='+5V')
for c in kinds['CONN40']:check(c['ref']+' motor temperature transferred on pin38',actual[c['ref'],'38']=='TEMP_MOTOR')
...
erc=json.loads((root.parent/'verification/erc_b1.json').read_text(encoding='utf-8'))
violations=[v for s in erc['sheets'] for v in s['violations']]
check('KiCad ERC zero violations',not violations)
fp=parse((root/'ESC_B1.pretty/TI_RTA0040B_6x6_P0.5_EP4.15.kicad_mod').read_text(encoding='utf-8'))
...
for p in parts:
 ...
 if p['footprint']:
  lib,name=p['footprint'].split(':');f=(root/(lib+'.pretty') if lib=='ESC_B1' else Path('C:/Program Files/KiCad/10.0/share/kicad/footprints')/(lib+'.pretty'))/(name+'.kicad_mod')
...
(root.parent/'verification/b1_checks.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
